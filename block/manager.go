...
func NewManager(
	...
	lastBatchDataBytes, err := store.GetMetadata(ctx, types.LastBatchDataKey)
	...
	if height, err := m.store.GetMetadata(ctx, types.DAIncludedHeightKey); err == nil && len(height) == 8 {
...
func (m *Manager) retrieveBatch(ctx context.Context) (*BatchData, error) {
	...
	if res != nil && res.Batch != nil {
		...
		if err := m.store.SetMetadata(ctx, types.LastBatchDataKey, convertBatchDataToBytes(res.BatchData)); err != nil {
