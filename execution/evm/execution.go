...
func (c *EngineClient) GetTxs(ctx context.Context) ([][]byte, error) {
	...
	err := c.engineClient.CallContext(ctx, &payloadResult, "engine_getPayloadV4", c.payloadID)
	c.payloadID = nil
	if err != nil {
		return nil, fmt.Errorf("engine_getPayloadV4 failed: %w", err)
	}
...
func (c *EngineClient) getBlockInfo(ctx context.Context, height uint64) (common.Hash, common.Hash, uint64, error) {
